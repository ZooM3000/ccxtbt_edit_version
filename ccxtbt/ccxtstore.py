...
class CCXTStore(with_metaclass(MetaSingleton, object)):
    ...
    def retry(method):
        ...
        def retry_method(self, *args, **kwargs):
            for i in range(self.retries):
                ...
                time.sleep(self.exchange.rateLimit / 700) #1000
                try:
    ...
    @retry
    def get_balance(self, cur = 'USDT'):
        #print ('get balance store')
    ...
    def getposition(self):
        #print ('getposition')
        dataname = ['ETH/USDT', 'BAT/USDT', 'LUNA/USDT', 'MATIC/USDT', 'SOL/USDT', 'GALA/USDT', 'AVAX/USDT', 'DOT/USDT', 'XRP/USDT', 'NEAR/USDT']
        cur = ['ETH', 'BAT', 'LUNA', 'MATIC', 'SOL', 'GALA', 'AVAX', 'DOT', 'XRP', 'NEAR']
        sym = ['ETHUSDT', 'BATUSDT', 'LUNAUSDT', 'MATICUSDT', 'SOLUSDT', 'GALAUSDT', 'AVAXUSDT', 'DOTUSDT', 'XRPUSDT', 'NEARUSDT'] #name
        bal = self.exchange.fetch_balance()
        for i in range(len(dataname)):
            tr = self.exchange.fetch_my_trades(symbol = sym[i])
            #print (tr)
            if len(tr) != 0:
                price = tr[-1]['price']
            size = bal['free'][cur[i]]
            if size==0.0:
                price = 0.0
            self.positions[dataname[i]] = Position(size, price)
            #print (dataname[i], self.positions[dataname[i]])
