...
class CCXTBroker(with_metaclass(MetaCCXTBroker, BrokerBase)):
    ...
    def start(self):
        ...
        self.startingcash = self.store._cash = self.getcash()  # Стартовые и текущие свободные средства по счету
        self.startingvalue = self.store._value = self.getvalue()  # Стартовый и текущий баланс счета
        self.store.getposition()  # То получаем их
    
    def get_balance(self):
        print ('get balance broker')
    ...
    def getvalue(self, data=None, cur='USDT'):
        # return self.store.getvalue(self.currency)
        #print ('get val broker')
        #print (str(data))
        self.store.get_balance(cur)
        self.value = self.store._value
        return self.value
